# PROGRESS NOTES — áp dụng system_adjustments_v3.md vào mã nguồn
...
## 1. Việc đã làm (map theo đúng thứ tự ưu tiên §8 của system_adjustments_v3.md)
...
## 2. Việc chủ động **bỏ qua** (đúng theo chỉ định của system_adjustments_v3.md)
...
Không còn mục hành động cụ thể nào trong `system_adjustments_v3.md` bị bỏ sót.
...
**Không đổi / không cần sửa** (đã đúng theo `system_adjustments_v3.md`, hoặc không nằm trong phạm vi tài liệu):
...
Không có mục nào trong `system_adjustments_v3.md` còn ở trạng thái "chưa làm". Nếu muốn mở rộng thêm (ngoài phạm vi tài liệu gốc), các hướng tự nhiên tiếp theo là:
